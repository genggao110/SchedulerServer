...
var RunModels = function(editorUi){
	var contentDiv = document.createElement('div');
	var title =  document.createElement('h3');
	title.innerHTML = "Models running...";
	contentDiv.appendChild(title);
	var runModelDiv=document.createElement('div');
	runModelDiv.style.overflowY = "auto";
	runModelDiv.style.height = "200px";
	...
	downloadBtn.innerHTML = "Download result";
	downloadBtn.className = 'geBtn gePrimaryBtn';
	...
	var runTimer =  setInterval(function(){
		$.ajax({
			// url: "http://172.21.213.33:8080/computableModel/curResult"+"?objectId="+window.runModelUid,
			url: "http://172.21.213.33:8080/computableModel/curResult?objectId=123454",
			type: "GET",
			async: false,
			success: function (data) {
				var curResultList  = data;
				var finalIndex = curResultList.length-1;
				// var finalEventId = curResultList[finalIndex].event.id;
				if(lastEventId!='end'){
					var curIndex = 0;
					for(var i=0;i<curResultList.length;i++){
						var infoItem = curResultList[i];
						var curEventId = infoItem.event.id;
						if(curEventId == lastEventId){
							curIndex = i;
							break;
						}
					}
					for(var i=curIndex;i<curResultList.length;i++){
						var infoItem = curResultList[i];
						var curModelId = infoItem.model.id;
						var curStateId = infoItem.state.id;
						var curEventId = infoItem.event.id;
						
						if(curEventId=='end'){
							cancelBtn.style.visibility = 'visible';
							downloadBtn.style.visibility = 'visible';
							title.innerHTML="Models run over"
							window.clearInterval(runTimer);
						}
						else{
							if(lastModelId!=curModelId){
								lastModelId = curModelId;
								var modelTitle = document.createElement('h3');
								modelTitle.innerHTML ="Model: " + infoItem.model.name;
								runModelDiv.appendChild(modelTitle);
								runModelDiv.scrollTop = runModelDiv.scrollHeight;
							}
							if(lastStateId!=curStateId){
								lastStateId = curStateId;
								var stateTitle = document.createElement('p');
								stateTitle.innerHTML = "State: " + infoItem.state.name;
								stateTitle.style.marginLeft = '5px';
								runModelDiv.appendChild(stateTitle);
								runModelDiv.scrollTop = runModelDiv.scrollHeight;
							}
							if(lastEventId!=curEventId){
								lastEventId=curEventId;
								var itemDiv = document.createElement('div');
								itemDiv.style.margin="5px 0 5px 10px";
								itemDiv.innerHTML='<span>'+ 'Event name: '+infoItem.event.name +'</span>'+'<a style="float:right;margin-right:5px" href="'+infoItem.event.url+'">'+'data download'+'</a>';
								runModelDiv.appendChild(itemDiv);
								runModelDiv.scrollTop = runModelDiv.scrollHeight;
							}
						}
					}
			...
			error: function (err) {
				console.log("Get running info fail.");
				window.clearInterval(runTimer);
				editorUi.hideDialog();
			}
		});
	},1000);
	contentDiv.appendChild(runModelDiv);
