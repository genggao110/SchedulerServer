...
//    }
    ...
    public JSONArray getInvite(String userName){
        JSONArray jsonArray = new JSONArray();

        List<Invite> inviteList = inviteDao.findByInvited(userName);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        for(int i =0 ;i<inviteList.size();i++){
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("oid",inviteList.get(i).getOid());
            jsonObject.put("project",inviteList.get(i).getProject());
            jsonObject.put("title",projectService.getByOid(inviteList.get(i).getProject()).getTitle());
            jsonObject.put("model",inviteList.get(i).getModel());
            jsonObject.put("modelName",inviteList.get(i).getModelName());
            jsonObject.put("createDate",sdf.format(inviteList.get(i).getCreateDate()));
            jsonObject.put("status",inviteList.get(i).getStatus());

            JSONObject user = portalUserService.getUsersFromPortalByType(inviteList.get(i).getInvite(),"userName").getJSONObject(0);


            jsonObject.put("inviteSender",user); //放入邀请者整个对象
    ...
    public int acceptInvite(String oid){
        Invite invite = inviteDao.findByOid(oid);

        invite.setStatus(1);
        projectService.changeInvitingUser(invite.getProject(),invite.getInvited());
